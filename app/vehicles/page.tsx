...
export default async function VehiclesPage({
  ...
  let filteredCars = allCars.filter((car) => {
    ...
    const carPrice =
      typeof car.price === "number"
        ? car.price
        : Number.parseInt(car.price.toString());
    ...
    // Filter by condition
    if (conditionFilter && car.condition !== conditionFilter) {
      return false;
    }
    ...
    filteredCars = [...filteredCars].sort((a, b) => {
      const priceA =
        typeof a.price === "number"
          ? a.price
          : Number.parseInt(a.price.toString());
      const priceB =
        typeof b.price === "number"
          ? b.price
          : Number.parseInt(b.price.toString());
  ...
  const conditions = [...new Set(allCars.map((car) => car.condition))].sort();
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <p className="text-gray-600 mb-8">
        Sélectionnez la catégorie, la marque, le modèle et différentes autres
        critères spécifiées pour obtenir la voiture de vos rêves.
      </p>
